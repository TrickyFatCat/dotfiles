...
def --env main [dir: string] {
    ...
    let session_name = "develop" | append ($dir | path basename) | str join "-"

    let result = ^zellij list-sessions -s | complete
    let session_exists = (
        $result.exit_code == 0
        and ($result.stdout | lines | any {|s| $s == $session_name})
    )

    let args = if $session_exists {
        ["attach" $session_name]
    } else {
        ["--new-session-with-layout" "default-dev" "--session" $session_name]
    }

    open-terminal --dir=$dir --detached --command=zellij --args=$args
